...
void run_scheduler(workload_item *workload, size_t workload_size)
{
    ...
    for (int t = 0; t <= max_timestep; t++)
    {
        ...
        for (int i = 0; i < workload_size; i++)
        {
            if (workload[i].tf < t && contains_pid(running_queue, workload[i].pid))
            {
                workload_item current_workload_item = workload[i];
                printf("  > process pid=%d prio=%d (%s) finished after time t=%zu \n", current_workload_item.pid, current_workload_item.prio, current_workload_item.cmd, current_workload_item.tf);
                remove_from_queue(&running_queue, current_workload_item.pid);
                continue; // Process is finished
            }
            
            else if (workload[i].ts <= t && workload[i].tf > t)
            {
